...
import 'package:intl/intl.dart'; // Homework: Format Tanggal
import 'package:intl/date_symbol_data_local.dart'; // Homework: Lokalisasi ID
...
class _LogbookViewState extends State<LogbookView> {
  ...
  // Homework: Fungsi Format Tanggal Indonesia
              ...
              child: FutureBuilder<List<LogModel>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    // Homework: Connection Guard (Offline Mode Warning)
                    return ListView(
                      children: [
                        SizedBox(height: MediaQuery.of(context).size.height * 0.2),
                        const Icon(Icons.wifi_off_rounded, size: 70, color: Colors.orange),
                        const Padding(
                          padding: EdgeInsets.all(20.0),
                          child: Text(
                            "Koneksi terputus! Pastikan internet aktif dan tarik layar ke bawah untuk mencoba lagi.",
                            textAlign: TextAlign.center,
                            style: TextStyle(color: Colors.grey, fontSize: 16),
                          ),
                        ),
                        Center(
                          child: ElevatedButton(onPressed: _refresh, child: const Text("Coba Lagi")),
                        )
                      ],
                    );
                    ...
                    return ListView(
                      children: [
                        SizedBox(height: MediaQuery.of(context).size.height * 0.2),
                        const Center(child: Text("Data tidak ditemukan di Atlas.")),
                      ],
                    );
                  }
                  ...
                  return ListView.builder(
                    physics: const AlwaysScrollableScrollPhysics(), // Memastikan Pull-to-Refresh selalu jalan
